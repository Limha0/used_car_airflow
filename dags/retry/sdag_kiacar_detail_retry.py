...
@dag(
    dag_id="sdag_kiacar_detail_retry",
    schedule=None,
    start_date=pendulum.datetime(2026, 3, 1, tz="Asia/Seoul"),
    catchup=False,
    max_active_runs=1,    # 체인 사이클 중첩 방지
    tags=["used_car", "kiacar", "detail", "retry"],
)
def kiacar_detail_retry():
    """
    기아차 상세 재수집:
    ...
    def fetch_retry_targets() -> list[dict[str, str]]:
        ...
        ORDER BY l.model_sn
        """
        logging.info("kiacar detail retry select_stmt ::: %s", sql)
        ...
        logging.info("kiacar detail retry 재수집 대상: %d건", len(rows))
        if not rows:
    ...
    def crawl_and_save_csv(target_rows: list[dict[str, str]]) -> str:
        ...
        csv_path = output_dir / f"kiacar_detail_retry_{run_ts}.csv"
        logging.info("kiacar detail retry 출력 CSV: %s", csv_path.resolve())
        ...
        logging.info("kiacar detail retry 재수집 처리 시작 — 총 건수: %d", total)
        ...
        with sync_playwright() as p:
            ...
            for idx, row in enumerate(target_rows, 1):
                ...
                try:
                    detail_data, fail_reason = _crawl_one(page, idx, product_id, detail_url, per_detail_dir)
                    if detail_data:
                    ...
                    try:
                        ...
                        # logging.info(
                        #     "kiacar detail retry update_result ::: product_id=%s complete_yn=%s updated_rows=%d",
                        #     product_id,
                        #     yn,
                        #     int(n or 0),
                        # )
                ...
                if idx % recycle_every == 0 and idx < total:
                    ...
                    page = context.new_page()
                    page.add_init_script("Object.defineProperty(navigator, 'webdriver', {get: () => undefined})")
                    logging.info("kiacar detail retry browser context 재생성 완료: processed=%d/%d", idx, total)
        ...
        logging.info(
            "✅ kiacar detail retry 완료: collected=%d failed=%d skipped=%d total=%d csv=%s",
            collected,
            failed,
            skipped,
            total,
    ...
    def load_retry_csv_to_ods(csv_path: str) -> dict[str, Any]:
        ...
        if not rows:
            logging.info(
                "kiacar detail retry: CSV 데이터 행 없음 → Detail INSERT 생략, List complete_yn 동기화만. csv=%s",
                p,
            )
        ...
        table_count = CommonUtil.get_table_row_count(hook, TARGET_DETAIL_TABLE)
        logging.info(
            "kiacar detail retry CSV → ODS 적재 완료: table=%s, inserted_rows=%d, table_count=%d, csv=%s",
            TARGET_DETAIL_TABLE,
            len(rows),
            table_count,
            p,
    ...
    trigger_next_cycle_list = TriggerDagRunOperator(
        ...
        trigger_dag_id="sdag_kiacar_crawler",
...
dag_object = kiacar_detail_retry()
...
def _download_image(page, image_url: str, save_path: Path) -> bool:
    ...
    try:
        headers = {
            "Referer": (page.url or "https://www.kia.com/").split("#")[0],
            "User-Agent": (
...
def _crawl_one(page, idx: int, product_id: str, detail_url: str, detail_img_dir: Path) -> tuple[dict[str, Any] | None, str]:
    """
    기아차 단일 상세페이지 크롤링(재수집용 간소화 버전).
    - 원본 DAG의 핵심 필드(가격/스펙/옵션/기록/이미지)를 우선 채움.
    - 실패 시 (None, reason) 반환.
    """
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                logging.warning("kiacar detail retry 재시도 (%d/3): %s - %s", attempt + 2, product_id, e)
                time.sleep(2)
            else:
    ...
    if root.count() == 0:
        return None, "DOM_없음_.buy-car-detail"
    ...
    try:
        ...
        data["car_installment"] = _safe_text(price_block.locator(".calc-payment .calc-payment__detail"))

        spec_items = root.locator(".buy-car-detail__car-spec .car-spec__list--info .car-spec__item")
        spec_keys = ["car_num", "car_type", "car_seat", "car_engine", "car_ext_color", "car_int_color"]
        ...
        # 외장360(1장) + 내장360(1장) + 차량사진(전부) 이미지 수집
        try:
            img_urls: list[str] = []
            ...
            logging.info("기아 이미지 URL 수집: %d건", len(img_urls))
            saved = 0
