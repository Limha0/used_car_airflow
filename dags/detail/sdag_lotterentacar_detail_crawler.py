...
@dag(
    dag_id="sdag_lotterentacar_detail_crawler",
    schedule=None,
...
def lotterentacar_detail_crawl():
    """롯데렌터카 상세: 최신 date_crtr_pnttm 스냅샷 기준 register_flag='A' 만 수집 → CSV → Detail ODS 적재 → List complete_yn 동기화."""

    @task
    def fetch_target_urls() -> list[dict[str, str]]:
        """
        ods.ods_car_list_lotterentacar 에서
        ...
        WHERE TRIM(COALESCE(l.register_flag::text, '')) = 'A'
        ...
        try:
            with conn.cursor() as cur:
                ...
                logging.info(
                    "롯데렌터카 detail 수집 기준 date_crtr_pnttm(최신): %s",
                    latest_pnttm,
                )
                cur.execute(sql)
                ...
                if not rows and latest_pnttm is not None:
                    cur.execute(
                        ...
                        SELECT
                            ...
                            COUNT(*) FILTER (
                                WHERE TRIM(COALESCE(register_flag::text, '')) = 'A'
                            ) AS cnt_a,
                            COUNT(*) FILTER (
                    ...
                    if diag:
                        ...
                        if cnt_a == 0:
                            logging.info(
                                "롯데렌터카 상세: 최신 적재일(%s) 기준 신규(register_flag=A) 0건 → "
        ...
        if not rows:
            logging.info(
                "수집 대상 URL이 없습니다. 테이블/조건(register_flag = 'A', detail_url) 확인 또는 신규(A) 0건."
            )
        return rows
    ...
    def summarize_targets(target_rows: list[dict[str, str]]) -> list[dict[str, str]]:
        ...
        logging.info("롯데렌터카 상세 크롤 준비: 총 %d건, detail_url 있음 %d건", n, with_url)
        if not target_rows:
    ...
    def crawl_and_save_csv(target_rows: list[dict[str, str]]) -> str:
        ...
        policy = CommonUtil.DETAIL_LIST_COMPLETE_FLAG_POLICY_LATEST_SNAPSHOT
        ...
        if total == 0:
            with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
                ...
                w.writeheader()
            logging.info(
                "롯데렌터카 상세: 수집 대상 0건 — Playwright 생략, 헤더만 기록: %s",
                csv_path,
            )
            return str(csv_path)
        ...
        with sync_playwright() as p:
            ...
            for idx, row in enumerate(target_rows, 1):
                ...
                CommonUtil.clear_image_files(per_dir)
    ...
    def load_detail_csv_to_ods(csv_path: str) -> dict[str, Any]:
        """
        crawl_and_save_csv 결과 CSV를 ods.ods_car_detail_lotterentacar로 적재.
        ...
        if not rows:
            logging.info(
                "롯데렌터카 상세 DAG 정상 완료: 신규(register_flag=A) 차량 없음 → detail INSERT 생략, "
        ...
        table_count = CommonUtil.get_table_row_count(hook, TARGET_DETAIL_TABLE)
        logging.info(
            "롯데렌터카 detail CSV 적재 완료: table=%s, inserted_rows=%d, table_count=%d, csv=%s",
            TARGET_DETAIL_TABLE,
            len(rows),
            table_count,
            p,
    ...
    trigger_retry = TriggerDagRunOperator(
        task_id="trigger_lotterentacar_detail_retry",
        trigger_dag_id="sdag_lotterentacar_detail_retry",
...
dag_object = lotterentacar_detail_crawl()
...
def _read_csv_rows(csv_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logging.exception("CSV 읽기 실패: %s", csv_path)
        raise
    return rows
...
# ───────────────────────────────────────────────
#  이미지 수집: page HTML → URL 추출 → requests 다운로드
# ───────────────────────────────────────────────

def _strip_dims_resize(url: str) -> str:
    """/dims/ 이후 전부 제거 → 원본 이미지 URL."""
    return _re.sub(r"/dims/.*$", "", url.strip())


def _fetch_lotte_detail_image_urls(page) -> list[str]:
    """
    Playwright 브라우저에서 img.src 로 이미지 URL 수집.
    image_downloader.py 의 fetch_browser_detail_images 방식 동일.
    """
    # 차량 이미지 리스트가 렌더링될 때까지 대기
    try:
        page.wait_for_selector("ul.image-list li img", timeout=10_000)
    except Exception:
        logging.warning("롯데카 이미지 리스트 렌더링 대기 실패 (ul.image-list li img)")
        try:
    ...
    try:
            ...
            r"""() => {
                ...
                document.querySelectorAll('img').forEach(img => {
                    ...
                    if (src && !src.includes('icon') && !src.includes('logo') &&
                        !src.includes('svg') && !src.includes('loading') &&
                        !src.includes('placeholder') && !src.includes('banner') &&
                        (src.includes('mycarsave') || src.includes('uploadFile') ||
                         src.includes('LFILE'))) {
                        imgs.add(src);
                    }
                });
        ...
        logging.warning("롯데카 이미지 URL evaluate 예외: %s", e)
        raw_urls = []
    ...
    for raw in (raw_urls or []):
        original = _strip_dims_resize(raw)
        if original and original not in seen:
            seen.add(original)
            urls.append(original)

    logging.info("롯데카 이미지 URL 수집(img.src): %d건", len(urls))
    return urls


def _download_lotte_detail_images(
    product_id: str,
    image_urls: list[str],
    detail_img_dir: Path,
    referer: str,
    ...
    """
    requests 세션으로 이미지를 순차 다운로드한다.
    저장: detail/{product_id}/{product_id}_001.png, _002.png, ...
    반환: 성공 건수
    """
    if not image_urls:
        return 0
    ...
    session.headers.update({
        "Referer": referer or "https://tcar.lotterentacar.net/",
        "User-Agent": (
    ...
    for seq, url in enumerate(image_urls, start=1):
        save_path = detail_img_dir / f"{product_id}_{seq:03d}.png"
        try:
            resp = session.get(url, timeout=30)
            if resp.status_code != 200:
                logging.debug("이미지 HTTP %d: %s", resp.status_code, url)
                continue

            ct = (resp.headers.get("Content-Type") or "").lower()
            if "text/html" in ct or "application/json" in ct:
                logging.debug("이미지 아닌 응답(Content-Type=%s): %s", ct, url)
                continue

            body = resp.content
            if not body or len(body) < 1000:
                logging.debug("이미지 크기 작음(%d bytes): %s", len(body) if body else 0, url)
                continue

            save_path.write_bytes(body)
            saved += 1
        except Exception as e:
            logging.debug("이미지 다운로드 예외: %s → %s", url, e)
...
def _crawl_one(
    ...
    try:
        ...
        # ── 이미지 수집: HTML에서 URL 추출 → requests 세션으로 다운로드 ──
        try:
            image_urls = _fetch_lotte_detail_image_urls(page)
            referer = (page.url or "").split("#")[0].strip()
            saved_count = _download_lotte_detail_images(
                product_id, image_urls, detail_img_dir, referer,
            )
            logging.info(
                "롯데카 이미지 저장: product_id=%s, 추출=%d건, 저장=%d건, 경로=%s",
                product_id, len(image_urls), saved_count, detail_img_dir,
            )
        except Exception as e:
            logging.warning("롯데카 이미지 저장 실패 product_id=%s: %s", product_id, e)
